...
class BackplaneAPI {
  ...
  async addOrg(code, displayname, license, owner) {
    try {
  ...
  async addProduct(code, displayname, platformId, ownerId, orgId) {
    try {
      let productObj = {
        code,
        name: displayname,
  ...
  async registerUser(displayname, email, password, orgId, userType) {
    try {
      let userObj = {
        name: displayname,
        email,
  ...
  async addRequest(requestType, requestedForType, requestedForId) {
    try {
      let requestObj = {
        requestType,
        requestedForType,
